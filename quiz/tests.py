...
class QuizModelTestCase(TestCase):
    ...
    def test_quiz_creation(self):
        quiz = Quiz.objects.create(user=self.user, quiz_type="national", score=80)
    ...
    def test_quiz_str_representation(self):
        quiz = Quiz.objects.create(user=self.user, quiz_type="clubs", score=90)
    ...
    def test_quiz_save_existing_quiz(self):
        Quiz.objects.create(user=self.user, quiz_type="stadium", score=70)

        quiz = Quiz(user=self.user, quiz_type="stadium", score=70)
        quiz.save()
    ...
    def test_quiz_save_new_quiz(self):
        quiz = Quiz(user=self.user, quiz_type="wc", score=85)
        quiz.save()
