...
def penalty_goal(answer):
    ...
    rank_table = home_penalty.add(away_penalty, fill_value=0).sort_values(ascending=False)
    return calculate_score(answer[1], rank_table, top=30)
...
def conceded_goal(answer):
    ...
    rank_table = home_conceded.add(away_conceded, fill_value=0).sort_values(ascending=False)
    return calculate_score(answer[1], rank_table, top=30)
